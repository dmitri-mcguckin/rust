...
pub fn u64_u32(x: u64) -> u32 { u32::try_from(x).unwrap() }
pub fn u32_u64(x: u32) -> u64 { u64::try_from(x).unwrap() }

fn write_key(filename: &str, key: u64) {
	// Open the key-file
	match File::create(filename) {
		Ok(f) => f,
		Err(e) => {
			println!("There was a problem creating the file: {}\n\tError: {}", filename, e);
			process::exit(-1);
		},
	};
	...
	match fs::write(filename, key.to_string()) {
		Ok(f) => f,
		Err(e) => {
			println!("There was a problem writing to the file: {}\n\tError: {}", filename, e);
			process::exit(-1);
		},
	};
}

fn read_key(filename: &str) -> u64 {
	// Open the key-file
...
fn genprime() -> u32 {
	...
	while x < E && gcd(u32_u64(x), u32_u64(E)) != 1 { x = rsa_prime(); }
	x
}
